...
class ProjectShow extends React.Component {
  ...
  render() {
    ...
    const step = this.props.steps.map(step => (
      <StepsIndexContainer step={step} key={step.id} />
    ));

    const comment = this.props.comments.map(comment => (
      <CommentsIndexContainer comment={comment} key={comment.id} />
    ));
    ...
    return (
      <div className="proj-wrapper">
        ...
        {this.props.project.author_id === this.props.session.id ? (
          <div className="add-step-div">
            <button id="add-step" onClick={this.handleAddStep}>
              Add Steps
            </button>
          </div>
        ) : (
